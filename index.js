...
async function starts() {
	...
	Lxa.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color('escanea el codigo pero rapido'))
	})
...
hola 
*${mdata.subject}*que bueno verte por aca espero te la pases super en este grupo recuerda leer las reglas para no tener mal entendidos 

hola usa ${prefix}verify para empezar a usar el bot.`
				Lxa.sendMessage(mdata.id, teks, MessageType.text, { contextInfo: {"mentionedJid": [num]}})
			} else if (anu.action == 'remove') {
				num = anu.participants[0]
				teks = `Que bueno que te fuiste pedazo de aborto 😈
        @${num.split('@')[0]} ya no te queriamos ojala nunca regreses 👋`
				Lxa.sendMessage(mdata.id, teks, MessageType.text, {contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
  ...
  ferr: 'error intenta el rato😣',
  limitend: 'limite agotado.',
  error: {
  stick: '*☒*error intenta al rato',
  Iv: '*🛹* error en el link'
  },
  only: {
    group: '*☒* Funcion solo para *GRUPOS*',
    ownerG: '*☒* solo paera el *DUEÑO DEL GRUPO*',
    ownerB: '*☒* SOLO PARA EL *DUEÑO DEL BOT*',
    admin: '*☒*solo puede ser usado por  *ADMIN GRUP*',
    Badmin: '*☒* Jadikan Lexa *ADMIN !*',
    daftarB: `hola usa *${prefix}verify* para empezar a usar el bot`
  }
}
...
case 'menu':
hasil = `        ────────────────
hola *${pushname}* intenta escribir ${prefix}menu
        ────────────────`
reply(hasil)
        break
}
...
‣ Prefijo:  *「 ${prefix} 」*
‣ Nombre: *SAMURAI-BOT*
‣ WAConnection: *samurai*
‣ Estado online: *${kyun(uptime)}*
‣ Modo: *ON*
‣ Groupo:  *${groupName}*
‣ Numro de usuarios : *${_registered.length} User*
‣ Numero de chat : *${totalchat.length} Chat*
...
┠⊷️ *${prefix}Bucin*
┠⊷️ *${prefix}Bijak*
┠⊷️ *${prefix}Chatprank (Texto1/Texto<<2)*
...
┠⊷️ *${prefix}liststik*
┠⊷️ *${prefix}listvid*
┠⊷️ *${prefix}Tupai (responder audio)
┠⊷️ *${prefix}Slow (responder audio)*
┠⊷️ *${prefix}Gemuk (responder audio)*
┠⊷️ *${prefix}Bass (responder audio)*
┗━━━━━━━━━━━━━━━━━━━━
*〘 EDITING 〙*
┏━━━━━━━━━━━━━━━━━━━━
┠⊷️ *${prefix}Gtav (Foto)*
...
┠⊷️ *${prefix}Kbbi (consukta)*
┠⊷️ *${prefix}Tl (Texto / código de idioma)*
┠⊷️ *${prefix}Fakta*
┠⊷️ *${prefix}Kodebahasa*
┠⊷️ *${prefix}Kodenegara*
┗━━━━━━━━━━━━━━━━━━━━
...
*〘 ISLAM 〙*
┏━━━━━━━━━━━━━━━━━━━━
┠⊷️ *${prefix}Alquran (numero)*
┠⊷️ *${prefix}Ngaji*
┠⊷️ *${prefix}Tafsir (consulta)*
┠⊷️ *${prefix}Sholat (zona)*
┗━━━━━━━━━━━━━━━━━━━━
*〘 WEATHER 〙*
┏━━━━━━━━━━━━━━━━━━━━
┠⊷️ *${prefix}Cuaca (zona)*
...
//--Info bot
  ...
  ‣ *dueño* : Samir@OFC
  ‣ *Prrfijo* : ${prefix}
  ‣ *Contactis bloqueados* : ${blocked.length}
  ‣ *bot activo* : ${kyun(uptime)}
...
if (args.length < 1) return Lxa.sendMessage(from, '*☒* ingrese el texto', text, {
  quoted: mek
})
var pc = body.slice(9)
try {
...
reply(teks.trim())
if (data.message) return reply(`Lo siento información de anime*${teks}no encontrado`)
...
//--searching chord
case 'chord':
  if (args.length < 1) return reply('ingrese la pregunta')
  if (!isRegister) return reply(mess.only.daftarB)
...
case 'jadwaltv':
  ...
  ch = body.slice(10)
  if (args.length < 1) return reply('*☒* Ingrese un nombre de canal tv')
  Lxa.updatePresence(from, Presence.composing)
  reply(mess.wait)
  try {
...
method: 'get'
  ...
  hasil = `*Calendario Tv* : ${ch} hari ini\n${n}`
  reply(hasil)
...
case 'alquran':
  ...
  if (args.length < 1) return reply('*☒* Ingrese el número de surah número de surahri -114')
...
method: 'get'
  ...
  teks = `
  〘  *${data.surah}*  〙
  Adalah surah ke ${data.no} en el Corán por el número de versos  (${data.jumlah_ayat} ayat)
  ──────────────────────
...
case 'cuaca':
  ...
  if (args.length < 1) return reply('Ingrese la zona')
  tels = body.slice(7)
  try {
...
case 'play':
  ...
  if (args.length < 1) return reply('Ingrese el título de una canción')
  reply(mess.wait)
  play = body.slice(6)
  try {
...
case 'ytmp3':
  ...
  infomp3 = `*Audio encontrado  !!!*\n‣ Título : ${anu.result.title}\n‣ Fuente : ${anu.result.source}\n‣ Tamaño : ${anu.result.size}\n\n_Enviando archivos por favor espere  _\n\n_JSi el video no aparece, descárguelo usted mismo usando el enlace a continuación  _\n‣ *link* : ${anu.result.link}`
...
case 'ytmp4':
  ...
  if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply('Formato de enlace incorrecto, use el enlace de youtube  ')
  if (anu.error) return reply(anu.error)
  infomp3 = `*Video encontrado  *\n‣ *Título* : ${anu.result.title}\n‣ *Fuente* : ${anu.result.source}\n‣ *Tamaño* : ${anu.result.size}\n\n_Envíe archivos por favor espere  u_\n\n_Si el video no aparece, descárguelo usted mismo usando el enlace a continuación  _\n‣ *link* : ${anu.result.url_video}`
  buffer = await getBuffer(anu.result.thumbnail)
...
//---Youtube search
				case 'ytsearch':
					...
					for (let i of anu.result) {
						teks += `*Título* : ${i.title}\n*Id* : ${i.id}\n*Publicada* : ${i.publishTime}\n*Duración* : ${i.duration}\n*Puntos de vista* : ${h2k(i.views)}\n=================\n`
					}
					reply(teks.trim())
...
case 'fb':
  ...
  if (anu.error) return reply(anu.error)
  infomp3 = `*Video Ditemukan*\n‣ *Judul* : ${anu.title}\n‣ *Publikasi* : ${anu.published}\n‣ *Ukuran* : ${anu.filesize}\n\n_Envíe archivos por favor espere, use el enlace de facebook_\n\n_Si el video no aparece, descárguelo usted mismo usando el enlace a continuación  _\n‣ *link* : ${anu.result}`
  lagu = await getBuffer(anu.result)
  Lxa.sendMessage(from, lagu, video, {
mimetype: 'video/mp4', filename: `${anu.result.title}.mp4`, quoted: mek
  })
...
case 'ig':
  ...
  lagu = await getBuffer(anu.result)
  Lxa.sendMessage(from, lagu, video, {
mimetype: 'video/mp4', filename: `SAMURAI.mp4`, quoted: mek
  })
...
//--harta tahta
				case 'tahta':
					if (args.length < 1) return reply('El texto es om')
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
						Lxa.sendMessage(from, await getBuffer(anu), image, { quoted: mek, caption: `*Tesoro del trono ${teks}*` })
					} else {
						reply('Hay un error')
					}
					break
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`❌ Falló, al convertir ${tipe} el stiker`)
							})
							.on('end', function () {
								console.log('Finish')
								exec(`webpmux -set exif ${addMetadata('Lexa', 'Mrf.zvx')} ${ran} -o ${ran}`, async (error) => {
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							fs.writeFileSync(ranp, buffer, (err) => {
								if (err) return reply('lo siento ocurrio un eror intenta al rato.')
							})
...
//-- temp
			...
			case 'addstik':
				...
				Lxa.sendMessage(from, ` Sticker agregado con exito\nComprobar${prefix}liststik`, MessageType.text, { quoted: mek })
				break
			...
			case 'addvn':
				...
				Lxa.sendMessage(from, `Adición de audio exitosa\nCek dengan cara ${prefix}listvn`, MessageType.text, { quoted: mek })
				break
			...
			case 'addimg':
				...
				Lxa.sendMessage(from, `Agregar videos con éxito \nCompruebe de alguna manera ${prefix}listimage`, MessageType.text, { quoted: mek })
				break
			...
			case 'listimg':
				...
				teks += `\n*Total : ${imagenye.length}*\nusar comandos\n*${prefix}getimg (nombre del paquete)* \npara tomar fotografías`
			...
			case 'addvid':
			  if (!isOwner) return reply(mess.only.ownerB)
				...
				Lxa.sendMessage(from, `Agregar video correctamente \nCompruebe registrándose${prefix}listvid`, MessageType.text, { quoted: mek })
				break
...
setTimeout(() => {
  Lxa.sendMessage(from, '_10 más segundos_', text) // ur cods
}, 20000) // 1000 = 1s,
setTimeout(() => {
  Lxa.sendMessage(from, '_20 más segundos_…', text) // ur cods
}, 10000) // 1000 = 1s,
setTimeout(() => {
...
//--Hilih maker
				...
				case 'tiktokstalk':
					try {
						if (args.length < 1) return Lxa.sendMessage(from, '¿Dónde está el nombre de usuario, eh?', text, {quoted: mek})
...
//--menaikan jabatan
      case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Promovido con éxito@${mentioned[0].split('@')[0]} Como administrador de grupo!`, mentioned, true)
...
//--menurunkan jabatan
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Degradar con éxito @${mentioned[0].split('@')[0]} Conviértete en miembro del grupo!`, mentioned, true)
...
tm = `verificación está completa ${prefix}menu para mostrar la lista del menú`
reply(tm)
break
...
break
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('ya se activo')
						welkom.push(from)
